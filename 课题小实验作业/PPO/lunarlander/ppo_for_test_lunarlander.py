...
class PPO():
    ...
    def load_weight(model):
        weight = torch.load(os.getcwd() + "/ppo_lunarlander.pth", map_location=torch.device('cpu'))
    ...
    def optimize_ppo(self, data):
        ...
        for i in range(self.optim_epochs):
            ...
            for batch in data_loader:
                ...
                loss_value = (cur_vpred - vtarg_b).pow(2).mean()
                # entropy_loss
                loss_entropy = -cur_entropies.mean()
                # actor - 反向优化
                self.actor_optimizer.zero_grad()
                actor_loss = loss_policy + self.coef_entpen * loss_entropy
                actor_loss.backward()
                self.actor_optimizer.step()
                # critic - 反向优化
                self.critic_optimizer.zero_grad()
                critic_loss = self.coef_vf * loss_value
