...
# 在将图片输入到神经网络之前，首先需要对图片进行预处理。
# 这里主要通过opencv的COLOR_BGR2GRAY和THRESH_BINARY将图片转成灰度并进行二值化处理，这样有利于提升计算速度。
# 同时，还需要将图片resize成80x80的形式，以便网络输入。
def preprocess(observation):
    observation = cv2.cvtColor(cv2.resize(observation, (80, 80)), cv2.COLOR_BGR2GRAY)
    ret, observation = cv2.threshold(observation, 1, 255, cv2.THRESH_BINARY)
    return np.reshape(observation, (1, 80, 80))

# 网络结构：3个卷积层+2个线性层，最后输出向量的维度维2
...
class BrainDQNMain(object):
    def save(self):
        ...
        torch.save(self.Q_net.state_dict(), os.getcwd() + "/dqn_bird.pth")

    def load(self):
        if os.path.exists(os.getcwd() + "/dqn_bird.pth"):
            print("load model param")
            self.Q_net.load_state_dict(torch.load(os.getcwd() + "/dqn_bird.pth"))
            self.Q_netT.load_state_dict(torch.load(os.getcwd() + "/dqn_bird.pth"))
