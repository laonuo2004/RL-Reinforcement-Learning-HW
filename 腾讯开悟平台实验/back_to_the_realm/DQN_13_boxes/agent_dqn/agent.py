...
class Agent(BaseAgent):
    ...
    def predict(self, list_obs_data):
        ret = self.algorithm.predict_detail(list_obs_data, exploit_flag=False)
        # print(f"predict ret: \n{ret}\n")
        return ret

    @exploit_wrapper
    def exploit(self, list_obs_data):
        ret = self.algorithm.exploit_Map(list_obs_data=list_obs_data)
        self.logger.info(f"\n\n\nnow_position = {list_obs_data[0].gridpos}, idx = {self.algorithm.idx}, action = {self.algorithm.dir_map[self.algorithm.idx][list_obs_data[0].gridpos]}")
        return ret
    ...
    def action_process(self, act_data):
        # print(f"act_data.move_dir: {act_data.move_dir}, use_talent: {act_data.use_talent}")
    ...
    def observation_process(self, raw_obs, preprocessor, state_env_info=None):
        ...
        def prt(list) :
            ...
            for i in range(50, -1, -1) :
                prt = ""
                for j in range(51) :
                    prt = prt + str(int(array[i][j]))
                print(prt)
            print(f"\n\n\n\n\n\n")
        # prt(obstacle_map)
        # print(f"{grid_pos}")
        # print(f"{buff_pos}")
        # print(f"len = {len(treasure_pos_list)}")
        # print(f"\n\n\n\n{treasure_pos_list}\n\n\n\n")
        # exit(0)

        # self.algorithm.update_Map_information(grid_pos, obstacle_map)
        ...
        # print(f"\n\n\n\n{grid_pos}\n\n\n\n")
        return ObsData(feature=feature_vec + feature_map, legal_act=legal_act, gridpos=(grid_pos.x, grid_pos.z)), remain_info
